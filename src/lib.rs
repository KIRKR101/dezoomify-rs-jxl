...
async fn dezoomify_source_pyramid(
    ...
    for (index, zoom_level) in levels.into_iter().enumerate() {
        ...
        canvas
            ...
            .await?;
        let state = dezoomify_level_into_buffer(args, zoom_level, &mut canvas, CancellationToken::new()).await?;
...
/// # Errors
///
/// Returns an error if the input cannot be resolved, no suitable level can be
/// selected, output setup fails, or the image cannot be downloaded and encoded.
pub async fn dezoomify(args: &Arguments) -> Result<PathBuf, ZoomError> {
    dezoomify_with_cancel(args, CancellationToken::new()).await
}

/// Same as [`dezoomify`], but allows the caller to control cancellation by
/// passing a [`CancellationToken`] through the download pipeline.
///
/// # Errors
///
/// Returns the same errors as [`dezoomify`], plus [`ZoomError::Cancelled`]
...
pub async fn dezoomify_with_cancel(
    ...
    let save_as = reserve_unique_outname(
        &output_file,
        &zoom_level.title(),
        &base_dir,
    ...
    dezoomify_level(args, zoom_level, tile_buffer, cancel).await?;
    Ok(save_as)
}
...
/// token with other concurrently-running operations.
///
/// # Errors
///
/// Returns the same errors as [`process_bulk`].
pub async fn process_bulk_with_cancel(
    ...
    info!("Found {} images to process in bulk mode", dezoomer_result.len());

    for (index, image) in dezoomer_result.into_iter().enumerate() {
        let image_title = image.title().unwrap_or_else(|| format!("Image_{}", index + 1).into()).to_string();
        let zoom_levels = match image.into_zoom_levels(&http).await {
            Ok(levels) => levels,
            Err(e) => {
                log::warn!("Failed to process image {} ('{}'): {}", index + 1, image_title, e);
        ...
        let zoom_level = match choose_level(zoom_levels, args) {
            Ok(level) => level,
            Err(e) => {
                log::warn!("Failed to choose a zoom level for image {} ('{}'): {}", index + 1, image_title, e);
                stats.record_failure();
                continue;
            }
        };
        ...
        let title_owned = zoom_level.title().unwrap_or_else(|| image_title.clone());
        let outfile_opt = indexed_outfile.clone();
        let save_as = match reserve_unique_outname(
            &outfile_opt,
            &Some(title_owned.clone()),
            &base_dir,
            zoom_level.size_hint(),
        ) {
            Ok(path) => path,
            Err(error) => {
                log::warn!("Failed to prepare output file for image {} ('{}'): {}", index + 1, image_title, error);
                stats.record_failure();
                continue;
            }
        };
        let tile_buffer = create_tile_buffer(save_as.clone(), args.compression, args.jxl_effort);
        match dezoomify_level(args, zoom_level, tile_buffer, cancel.clone()).await {
            Ok(()) => stats.record_success(),
            Err(ZoomError::PartialDownload { .. }) => stats.record_partial(),
            Err(e) => {
                log::warn!("Failed to process image {} ('{}'): {}", index + 1, image_title, e);
                stats.record_failure();
            }
...
pub async fn dezoomify_level(
    ...
    let mut canvas = tile_buffer;
    let state = dezoomify_level_into_buffer(args, zoom_level, &mut canvas, cancel).await?;
    validate_download_success(&state)?;
    finalize_canvas(&mut canvas).await?;
...
async fn dezoomify_level_into_buffer(
    ...
    let mut coordinator =
        download_state::TileDownloadCoordinator::new(&zoom_level, args, cancel)?;
