...
impl Encoder for PngEncoder {
    fn add_tile(&mut self, tile: Tile) -> io::Result<()> {
        // Capture metadata from the first tile that carries it, so that a failed/empty first
        // tile doesn't cause us to drop the ICC/EXIF profile.
        ...
        if let Some(ref mut pixel_streamer) = self.pixel_streamer {
            ...
            // Buffer tiles until we either see metadata (and can write the header) or finalize.
            self.pending_tiles.push(tile);
            if self.icc_profile.is_some() || self.exif_metadata.is_some() {
                // Clone the metadata options so we don't hold a borrow of self during the call.
