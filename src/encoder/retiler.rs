        ...
        for cur_pos in covered_tiles_positions {
            ...
            let finished_image = {
                ...
                if tmp_tile.is_complete() {
                    debug!(
                        "Removing completed tile of level {} at position {cur_pos}: {}",
                        self.original_size, tmp_tile_path.display()
                    );
                        ...
                        self.tile_image_cache.push(key, tile_img)
                    {
                        ...
                        {
                            warn!("Unable to spill retiler tile to {}: {e}", spill_path.display());
                        }
                    }
                    None
                }
